...
public class ReviewAssistant implements Runnable {
    ...
    private List openChanges(List<Entry<Account, Integer>> list) {
        List<Entry<Account, Integer>> temp = new LinkedList<>();

        for (int i = 0; i < list.size(); i++) {
            Account account = (Account) list.get(i).getKey();
            try {
                int openChanges = gApi.changes().query("status:open reviewer:" + account.getId().get()).get().size();
                list.get(i).setValue(openChanges);
            } catch (RestApiException e) {
                log.error("Error in openChanges", e);
            } catch (Exception e) {
                log.error("Error in openChanges", e);
            }
        }
