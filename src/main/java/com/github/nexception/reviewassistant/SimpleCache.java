...
/**
 * This implementation of Cache writes to the plugin's data directory ({gerrit url}/plugins/ReviewAssistant).
...
public class SimpleCache implements Cache {
    ...
    public Calculation fetchCalculation(RevisionResource resource) {
        ...
        try (BufferedReader reader = Files.newBufferedReader(file.toPath(), Charset.forName("UTF-8"))) {
            ...
            log.error("Could not read calculation file for {}", resource.getPatchSet().getRevision().get());
            log.error(e.toString());
        }
